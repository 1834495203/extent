...
const defaultPrompt = "忽略前面的说明。总结到目前为止故事中最重要的事实和事件和出场的人物（人物的信息需要明确标注出来）。如果你的记忆中已经存在一个总结或摘要，那么就以此为基础，用新的事实来扩展。将摘要限制为｛｛words｝｝个字或更少。你的回答应该只包括摘要。请用中文回答。你还需要生成该摘要的名字和关键字。请确保生成的摘要与前文连贯，不许有重复。";
...
function getLatestMemoryFromChat(chat) {
    ...
    for (let mes of reversedChat) {
        if (mes.extra && mes.extra.memory) {
            // console.log("getLatestMemoryFromChat: ", mes.extra.memory);
...
function onMemoryClearClick() {
    //  删除所有消息的 extra.memory
    const context = getContext();
    ...
    setMemoryContext('', false);
}

function onSummarizeToIndexClick() {
    const targetIndex = parseInt(String($('#memory_summarize_to_index').val()));
    if (isNaN(targetIndex)) {
        toastr.error('Please enter a valid number');
        return;
    }
    summarizeToIndex(targetIndex, false);
}

/**
 * 从最早已总结的消息开始，按targetIndex总结对应数量的内容
 * @param {number} targetNum The index of the last message to summarize (inclusive).
 * @param {boolean} quiet If an informational toast should be displayed.
 * @returns {Promise<string>} Summarized text.
 */
async function summarizeToIndex(targetNum, quiet) {
    if (extension_settings.memory.source === summary_sources.extras) {
        toastr.warning('Summarization to index is not supported for Extras API');
        return '';
    }
    ...
    // 验证索引
    if (isNaN(targetNum) || targetNum < 0 || targetNum >= totalMessages) {
        toastr.error(`Invalid index: ${targetNum}. Must be between 0 and ${totalMessages - 1}.`);
        return '';
    }
    ...
    // 创建总结上下文
    const prompt = await getSummaryPromptForNow(context, true, targetNum)

    // 获取之前的总结消息
    let before_summary = getLatestMemoryFromChat(context.chat);

    if (!prompt) {
        console.warn('Summarization prompt is empty. Skipping summarization.');
        return;
    }

    const { rawPrompt, lastUsedIndex } = await getRawSummaryPrompt(context, prompt, targetNum);

    const summary = await generateRaw(rawPrompt, '', false, false, prompt, extension_settings.memory.overrideResponseLength);
 ...
 * @param {number} msg_index 需要总结的对话数量
 * @returns {Promise<string>} Summary prompt or empty string
 */
async function getSummaryPromptForNow(context, force, msg_index=null) {
    ...
    // 从最新消息开始遍历，直到上一个已总结的消息
    for (let i = context.chat.length - 1; i >= 0; i--) {
        if (context.chat[i].extra && context.chat[i].extra.memory || i == 0) {
            // // 找到上一个已总结的消息，从该对话开始，按照msg_index进行总结(如果有)
            // if (msg_index) {
            //     if (i + msg_index - 1 >= context.chat.length) {
            //         console.debug('Not enough messages to summarize');
            //         return '';
            //     }
            //     messagesSinceLastSummary = 0;
            //     wordsSinceLastSummary = 0;
            //     for (let j = i; j <= i + msg_index; j++) {
            //         messagesSinceLastSummary++;
            //         wordsSinceLastSummary += extractAllWords(context.chat[j].mes).length;
            //     }
            // }
            break;
        }
...
async function summarizeChatMain(context, force, skipWIAN) {
    ...
    setMemoryContext(total_summary, true, index);
    return summary;
}
 ...
 * @param {number} msg_index Number of messages to summarize
 * @returns {Promise<{rawPrompt: string, lastUsedIndex: number}>} Raw summarization prompt
 */
async function getRawSummaryPrompt(context, prompt, msg_index = null) {
    /**
...
function setupListeners() {
    ...
    // 新增：计算计数器
    $('#memory_counters').off('click').on('click', onMemoryCountersClick);
